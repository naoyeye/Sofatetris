...
ge1doot.PHY2D = function () {
    ...
    // ==== BroadPhase 'Sweep and Prune' 1D Constructor ====
    ...
    var SAP = {
        ...
        SweepPoint: function (body, index) {
            this.body    = body;
            this.AABB    = body.motionBounds; // 运动边界
            this.axis    = body.motionBounds.axis; // 运动边界轴
            this.index   = index;
        ...
        del: function (rb) {
            ...
            var u = this.pairs.length;
            while (u--) {
                var q = this.pairs[u];
                if (q.A === rb.motionBounds || q.B === rb.motionBounds) {
                    this.pairs.splice(u, 1);
                }
            }
        },
